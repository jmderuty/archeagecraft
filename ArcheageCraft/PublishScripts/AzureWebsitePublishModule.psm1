...
<#
.SYNOPSIS
Crée un site Web Windows Azure.

.DESCRIPTION
Crée un site Web Windows Azure avec un nom et un emplacement spécifiques. Cette fonction appelle l'applet de commande New-AzureWebsite dans le module Azure. Si l'abonnement n'a pas encore de site Web avec le nom spécifié, cette fonction crée le site Web et retourne un objet de site Web. Si ce n'est pas le cas, elle retourne le site Web existant.

.PARAMETER  Name
Spécifie un nom pour le nouveau site Web. Le nom doit être unique dans Windows Azure. Ce paramètre est obligatoire.

.PARAMETER  Location
Spécifie l'emplacement du site Web. Les valeurs valides correspondent aux emplacements de Windows Azure, par exemple "Ouest des États-Unis". Ce paramètre est obligatoire.
...
<#
.SYNOPSIS
Crée une chaîne qui vous permet de vous connecter à une base de données SQL Windows Azure.

.DESCRIPTION
La fonction Get-AzureSQLDatabaseConnectionString assemble une chaîne de connexion pour se connecter à une base de données SQL Windows Azure.

.PARAMETER  DatabaseServerName
Spécifie le nom d'un serveur de base de données existant dans l'abonnement Windows Azure. Toutes les bases de données SQL Windows Azure doivent être associées à un serveur de base de données SQL. Pour obtenir le nom du serveur, utilisez la cmdlet Get-AzureSqlDatabaseServer (module Windows Azure). Ce paramètre est obligatoire.
...
<#
.SYNOPSIS
Crée des bases de données SQL Windows Azure à partir des valeurs du fichier de configuration JSON généré par Visual Studio.
