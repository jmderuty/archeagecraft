...
namespace ArcheageCraft.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // Pour plus d'informations sur la façon d'activer la confirmation du compte et la réinitialisation du mot de passe, consultez http://go.microsoft.com/fwlink/?LinkID=320771
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        // Pour plus d'informations sur la façon d'activer la confirmation du compte et la réinitialisation du votre mot de passe, consultez http://go.microsoft.com/fwlink/?LinkID=320771
                        // Envoyer un e-mail avec ce lien
                        // string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
                        // var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                        // SendEmail(user.Email, callbackUrl, "Confirmer votre compte", "Confirmez votre compte en cliquant sur ce lien");
        ...
        {
            // Pour des informations sur l'envoi de courrier, consultez http://go.microsoft.com/fwlink/?LinkID=320771
        }
